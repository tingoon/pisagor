...
Choose **closed**, **compound**, or **compound + shorthand** per component.

| Model | Use when | Public API | Barrel (`index.ts`) |
| ----- | -------- | ---------- | ------------------- |
| **Closed** | Single props surface; no subpart composition | `Foo` | `export * from "./foo"` |
...
- Utils and hooks may remain named exports outside the namespace. Do not re-export `{name}Recipe` from the component barrel — consumers import recipes from `@pisagor/recipes/<name>`.
...
- Type shorthand props with `Omit<FooRootProps, "children">` — no `children` on the default export.
- Prefer `<Foo title="…" />` over `<Foo.Root><Foo.Title>…</Foo.Title></Foo.Root>` when preset props are enough.
- Composition uses `Foo.Root` only when flexibility is required (`asChild`, custom markup, slot order); never nest `Foo.Title` / `Foo.Description` under `<Foo>`.
...
- Export a single function with convenience props and DOM sub-element `*Props` bags (content/behavior — not styling).
- **Closed multi-slot:** implement private Parts + `<name>.context.tsx` (`slots` from `@pisagor/recipes` `{name}Recipe`); compose in `#region Closed`. Do not export private parts from the barrel — only `Foo` / `FooProps`. Ark types used in public sub-element bags (`imageProps?: Omit<AvatarImageProps, …>`) **are** re-exported from the barrel (`export type { AvatarImageProps, AvatarFallbackProps } from "@ark-ui/react/avatar"`).
- Export via `export * from "./foo"` (shared package barrel only).
- Do **not** set `displayName` / `#region Display Names` for a single closed `export function Foo` — the function name is enough. Use Display Names only for compounds / shorthand / private parts that need `"Foo.Root"` / `"Foo.Part"` labels.
- Do **not** introduce a private `FooRootProps` alias when there is no `FooRoot` / compound part — put the primitive type on `FooProps` directly (`extends FooProps from Ark` / `ark.div` via `ComponentProps`, plus `FooVariantProps`).
- When the component adds **no** recipe, `data-*`, or behavior beyond Ark — re-export the Ark export directly (`export { Foo, type FooProps } from "@ark-ui/react/foo"`). Do not wrap in a passthrough function.

**Barrel (both):**

- Re-export types, hooks, or `RootProvider` only when part of the public API.
- Prefer owner `*Props` (`FooRootProps`, `FooCloseProps`) when wrapping or typing workspace components. Do not use `ComponentProps<typeof Foo>` / `ComponentProps<typeof Foo.Part>` for that.
- For Ark parts, prefer Ark’s exported `*Props` over `ComponentProps<typeof Primitive.Part>`. Reserve `ComponentProps` for `ark.*` / DOM and third-party components that do not export props.
- Pure Ark props: re-export from the **barrel** (`export type { FooRootProps } from "@ark-ui/react/foo"`). Component modules import for signatures only — do not `export type { FooRootProps }` there. When the local name must extend/omit the Ark type of the same name, import as `FooRootProps as FooPrimitiveRootProps` (mirror `Foo as FooPrimitive`).
- Do **not** add identity aliases (`type FooProps = FooPrimitiveRootProps` or empty `interface FooProps extends FooPrimitiveRootProps {}`). Import and use the Ark type directly. Rename aliases only when the public name differs (`ComboboxFieldInputProps = ComboboxPrimitiveInputProps`, `TreeViewItemTitleProps = TreeViewPrimitiveItemTextProps`).
- Barrel may alias for public names (`DatePickerRootProps as DatePickerProps`).
...
- Do not pass `children` to shorthand exports for manual subpart layout.
- Do not fall back to `children` in shorthand (`description ?? children`, `title ?? children`).
- Do not keep placeholder `tv()` slots (`""`, `[]`) in `@pisagor/recipes` to generate `classNames` keys. Empty strings in `variants` / `compoundVariants` are fine (no extra classes for that option).
- Do not type wrappers with `ComponentProps<typeof Foo.Part>` when that part exports `FooPartProps` / `FooCloseProps` (owner props).
...
| Pattern | Use |
| ------- | --- |
| **Compound + headless** | `{Name}Root` + subparts; headless primitive; `Object.assign` barrel |
...
In `FooShorthand` (and similar layout compose returns), leave a **blank line between each top-level slot** sibling in JSX. Nested containers group their own children without extra spacing.

---

## File regions

Every `<name>.tsx` file uses `#region` blocks in this order. Skip regions that do not apply; never reorder.
...
- Use **`#region Component`** (singular) when the file exports **one** component function.
- Use **`#region Parts`** (plural) when the file exports **two or more** component functions.
- Use **`#region Shorthand`** when the file exports `FooShorthand` — place it **after** `#region Parts`, not inside it.
- Use **`#region Closed`** for closed multi-slot compose (`Foo` that wires private parts) — place it **after** `#region Parts`, not inside it. Parts stay file-private; do not barrel-export them.
- Use **`#region Display Names`** when any export sets `displayName` — place it **last**, after Parts / Shorthand / Closed. Include every assignment in declaration order (parts first, then shorthand/closed). Spell the region exactly `Display Names` (with a space) — not `DisplayNames`. Skip the region for a single closed `export function Foo` (no `Foo.displayName = "Foo"`).
...
- Prefer Ark-exported part props (`AccordionRootProps`, `DialogContentProps`, `HighlightProps`) over `ComponentProps<typeof Primitive.Part>`. Use `ComponentProps<typeof ark.div>` (and native tags) when Ark does not export props.
- Re-export **unchanged** Ark props from the component **barrel** (`index.ts`): `export type { AccordionRootProps } from "@ark-ui/react/accordion"`. Do not `export type { AccordionRootProps }` from the component module — import there only for local signatures / `extends`.
- Library-owned props (`Omit` / `extends` with extra fields / `RefAttributes` / renamed aliases) stay in the component module and are re-exported from the barrel via `export type { FooProps } from "./foo"`.
- Do not invent passthrough aliases — if nothing is added, use the imported Ark `*Props` name as-is.
- When a local `FooRootProps` extends/omits Ark’s same-named type, import as `type FooRootProps as FooPrimitiveRootProps` (same `Primitive` placement as `Foo as FooPrimitive`).
- Ark `*Props` are typically `ComponentPropsWithoutRef` — when the wrapper destructures `ref`, intersect `RefAttributes<Element>` (what `ComponentProps<typeof Primitive>` used to provide).
- Note Ark naming quirks (e.g. tabs compound uses `TabListProps` / `TabTriggerProps` / `TabContentProps`, not `TabsListProps`).
...
| --- | --- | --- |
| Convenience prop conflicts with primitive prop | `type SelectRootProps = Omit<SelectPrimitiveRootProps<Item>, "collection" \| "onValueChange">;` | `type SelectRootProps = SelectPrimitiveRootProps<Item>;` + re-declare `onValueChange` with a different signature |
| Sub-element prop bags keep ownership of render/styling | `contentProps?: Omit<TooltipContentProps, "children" \| "className">` | `contentProps?: TooltipContentProps` (allows overriding `children`/`className`) |
| Preserve primitive contracts by default | `interface DialogBodyProps extends ComponentProps<typeof ark.div> {}` (no `Omit` needed) | `type DialogBodyProps = Omit<ComponentProps<typeof ark.div>, "role">` without an explicit conflict |

### Do not

- Do not type Ark wrappers with `ComponentProps<typeof Primitive.Part>` when Ark exports `*Props` for that part.
- Do not route styling through `*Props` bags — use `classNames` on multi-slot components only; single-element components use `className`.
- Do not type `classNames` as `VariantClassNames<FooRecipeSlot> & { … }` — use `VariantClassNames` only.
- `titleProps` / `contentProps` on shorthand APIs: behavior escape-hatches only (`Omit<…, "children" | "className">`).

---

## Headless wrapper

Behavior lives in the headless primitive — the styled layer adds visuals only.
...
- Forward `asChild`, portal, `lazyMount`, and `unmountOnExit` unchanged — keep Ark/Zag defaults; do not set library defaults for those presence props.
...
Recipes (`tv()`) are owned by **`@pisagor/recipes`**. Component packages import them; they do not call `tv()` or ship a `#region Variants`.
...
- **Root** always accepts **`className`** — never `rootClassName`.
- **Single-element recipes** (top-level `base:`): merge with `cn(fooRecipe({ … }), className)` or `fooRecipe({ …, className })` — no `classNames` prop. The call returns a **string**, not `{ base() }`.
- **Multi-slot recipes** (`slots:`): `const slots = fooRecipe()` (or `fooRecipe({ … })`); merge via `slots.part({ className: classNames?.part })`; type **`classNames`** as `VariantClassNames<{Name}RecipeSlot>` (`base` is omitted — root styling is always `className`).
- **Root slot:** `slots.base({ className })` — never `classNames?.base`.
- **Shell + slot:** keep outer `cn` only for a separate shell recipe — `cn(shellRecipe(…), slots.part({ className: classNames?.part }))`.
- Prefer `slots.part({ className })` over `cn(slots.part(), className)`.
...
- On **plain** styled nodes, set **`data-scope="{name}"`** and **`data-part="{part}"`** (root uses `data-part="root"`). On machine-backed parts, omit both — Ark/Zag already emit `data-scope` / `data-part`. Never override those attributes on a primitive part (Zag queries the DOM through them).
- Mirror variant props on root when useful (`data-variant`, `data-size`, `data-shape`).

### Authoring recipes (`@pisagor/recipes`)

When adding or editing a recipe module (not in the component file):

- Export `{name}Recipe` via `tv()`; use `slots:` for multi-part DOM, top-level `base:` for a single element; set `defaultVariants`.
- **`base:`** — tv default key. Single-element recipes use top-level `base:`; multi-slot recipes name the primary element slot `base` (not `root`). Dedicated shared shells may still use names like `inputRootRecipe`.
- **`slots:`** — only parts with real default styles; every slot key must have non-empty **default** class strings.
- **Do not** add placeholder slots (`""`, `[]`) to generate `classNames` keys — remove the slot and override per [Override without a slot](#override-without-a-slot).
- **Empty variant entries are fine** — `variants.*.*: ""` / `[]` and empty class strings in `compoundVariants` mean “no extra classes for this option.” That is not a placeholder slot.
- When a multi-export file names list chrome, use part names (`list`, not `base`) — e.g. `tabsListRecipe` uses `slots.list`.
- Split `tv()` when reuse needs a shared root shell (`inputRootRecipe`) separate from optional slots (`inputRecipe.clearableRoot`).
- Export `{Name}VariantProps`, `{Name}Recipe`, and `{Name}RecipeSlot` beside the recipe. Type context `slots` as `{Name}Recipe`.

### Override without a slot

When a part has no recipe slot by design:

1. **Compound part** — pass `className` on `{Name}Part` directly.
2. **`asChild`** — merge classes onto the child element.
3. **Remove** erroneous empty slots from the recipe `tv()` — do not keep placeholders for typing.
4. **Do not** add `*Props.className` or per-part styling bags.
