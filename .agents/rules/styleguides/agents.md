...
AI-facing doc for a directory: role, how to run work, local conventions — not a README duplicate.

| Location | Scope |
| -------- | ----- |
| **Root** | Monorepo index: workspace map, agent workflow — not rules/commands indexes |
| **Nested** (`apps/*`, `packages/*`, …) | That workspace only: commands, layout, conventions |

When adding a nested file, update the root [workspace map](../../../AGENTS.md).
...
- **Root:** short repo name (e.g. `# Monorepo`).
- **Nested:** one H1 with the workspace’s human-readable name in title case (e.g. `# Form Package`). Do not prefix with `AGENTS.md —`.

## Section catalog

Use only sections that apply. Omit empty ones.

| Section | Root | Nested | Purpose |
| ------- | :--: | :----: | ------- |
...
<Link `.env.example` when present; exceptions only — no duplicate var table.>

## Conventions

<Local patterns, import aliases.>
```

Add a **Commands** section only when needed (see [Section catalog](#section-catalog)).

Target length: ~50–90 lines unless the workspace is large.

## Upsert

When creating or refreshing `AGENTS.md`, preserve accurate existing sections; merge, do not wipe.
