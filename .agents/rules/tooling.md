...
- [Conventional Commits](https://www.conventionalcommits.org/); config: [`commitlint.config.ts`](../../commitlint.config.ts).
- **Scope:** workspace name when scoped (`feat(react-storybook):`, `fix(react):`); omit for cross-cutting changes.
- No `Co-authored-by:` / `Signed-off-by:` footers.
...
- **SSOT:** root [`package.json`](../../package.json) `packageManager`. Keep machine pins in lockstep with that field — do not hardcode a different Bun release in:
  ...
  - Root `@types/bun` (same major.minor.patch when published)
- Human docs (`README`, `CONTRIBUTING`, onboarding, …) say install Bun — **do not** repeat the version number there.
