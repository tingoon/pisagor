...
onMount(() => {
  unregister = ctx.registerNavigation();
});
onDestroy(() => {
  unregister?.();
});
...
$effect(() => {
  ...
  if (position !== "fixed" || !element) {
    ctx.setFixedStackVar(APP_SHELL_NAVIGATION_HEIGHT_VAR, "0px");
    return;
  }
  const syncHeight = () => {
    ctx.setFixedStackVar(APP_SHELL_NAVIGATION_HEIGHT_VAR, `${element.offsetHeight}px`);
  };
  syncHeight();
...
<nav
  ...
  style={`grid-area: navigation; ${style ?? ""}`}
  bind:this={el}
>
  {@render children?.()}
</nav>
