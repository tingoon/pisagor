...
export function TimerRoot(props: TimerRootProps): JSX.Element {
  ...
  return (
    <TimerContext value={{ slots: slots() }}>
      <TimerPrimitive.Root {...rest} class={slots().base({ class: cn(local.class) })}>
...
export function TimerArea(props: TimerAreaProps): JSX.Element {
  ...
  return <TimerPrimitive.Area {...rest} class={slots.area({ class: local.class })} />;
}

export function TimerItemGroup(props: TimerItemGroupProps): JSX.Element {
  const [local, rest] = splitProps(props, ["orientation", "children", "itemGroupRecipe", "class"]);
...
export function TimerItem(props: TimerItemProps): JSX.Element {
  ...
  return <TimerPrimitive.Item {...rest} class={slots.item({ class: local.class })} />;
}
...
export function TimerSeparator(props: TimerSeparatorProps): JSX.Element {
  ...
  const { slots } = useTimer();
  return (
    <TimerPrimitive.Separator {...rest} class={slots.separator({ class: local.class })}>
      {local.children ?? ":"}
...
export function TimerControl(props: TimerControlProps): JSX.Element {
  const [local, rest] = splitProps(props, ["class"]);
  const { slots } = useTimer();
  return <TimerPrimitive.Control {...rest} class={slots.control({ class: local.class })} />;
}

export function TimerActionTrigger(props: TimerActionTriggerProps): JSX.Element {
