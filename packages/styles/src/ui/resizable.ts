...
export const resizableEdgeHandleVariants = tv({
  ...
  variants: {
    handlePosition: {
      bottom: { root: "justify-end pb-2" },
      center: { root: "justify-center" },
      top: { root: "justify-start pt-2" },
    },
    placement: {
      end: { root: "inset-s-0" },
      start: { root: "inset-e-0" },
    },
  },
});
...
export const resizableInlineVariants = tv({
  base: "size-2.5",
});

export const resizableInline2Variants = tv({
  base: "size-2.5",
});
...
export const resizableResizeTriggerHandleVariants = tv({
  base: [
    "z-10 flex h-4 w-3 items-center justify-center rounded-xs border border-border bg-background text-muted-foreground transition-colors",
