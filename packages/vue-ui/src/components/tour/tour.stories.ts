...
const meta = preview.meta({
  ...
  subcomponents: {
    ...
    Backdrop: Tour.Backdrop,
    Body: Tour.Body,
    CloseTrigger: Tour.CloseTrigger,
  ...
  render: () => ({
    ...
    setup() {
      ...
      const addLog = (message: string) => {
        nextLogId += 1;
        logs.value = [...logs.value, { id: nextLogId, message }];
      };

      const onStatusChange = (details: { status: string }) => addLog(`Status: ${details.status}`);
      const onStepChange = (details: { stepId: string | null }) =>
        addLog(`Step changed: ${details.stepId ?? "unknown"}`);

      return { addLog, logs, onStatusChange, onStepChange, steps };
    },
    template: `
      <div class="flex flex-col gap-2">
        <Tour :onStatusChange="onStatusChange" :onStepChange="onStepChange" :steps="steps">
          <Tour.Trigger as-child>
            <Button variant="outline">Start tour</Button>
          </Tour.Trigger>
...
export const KeyboardNavigation = meta.story({
  render: () => ({
    components: { Button, PhKeyboard, Tour },
    setup() {
      const steps: TourStepType[] = [
        {
          actions: [{ action: "next", label: "Next" }],
...
export const Progress = meta.story({
  render: () => ({
    components: { Button, Tour, TourProgressBar },
    setup() {
      const steps: TourStepType[] = [
        {
          actions: [{ action: "next", label: "Next" }],
      ...
      <div class="flex flex-col gap-2">
        <Tour :steps="steps">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <Button id="btn-delete" size="sm" variant="outline">Delete</Button>
          </div>
          <Tour.Content>
            <Tour.Header>
              <Tour.ProgressText />
  ...
  render: () => ({
    ...
    setup() {
      ...
      const isNewItem = (index: number) =>
        index === items.value.length - 1 && items.value.length > 2;

      return { addItem, isNewItem, items, steps };
    },
    template: `
      <div class="flex flex-col gap-2">
        <Tour :steps="steps">
          ...
          <div class="flex flex-col gap-2">
            <Button id="btn-add-item" @click="addItem" size="sm" variant="outline">
              <PhPlus class="size-4" />
              Add Item
            </Button>
            <div class="flex flex-col gap-2">
...
export const WaitForInput = meta.story({
  render: () => ({
    components: { Checkbox, Field, Input, Tour },
    setup() {
