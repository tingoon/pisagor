...
export const dialogRecipe = tv({
  ...
  slots: {
    ...
    content: [
      ...
      "rounded-3xl border border-border/50 shadow-lg",
      "outline-hidden",
    ...
    title: ["font-heading font-semibold text-xl leading-tight tracking-tight"],
  },
  variants: {
    bottomStickOnMobile: {
      true: {
        content: [
          "max-sm:max-w-none",
          "max-sm:rounded-none max-sm:rounded-t-3xl max-sm:border-x-0 max-sm:border-t max-sm:border-b-0",
...
export type DialogRecipeFn = typeof dialogRecipe;
export type DialogVariantProps = VariantProps<DialogRecipeFn>;
export type DialogRecipe = ReturnType<DialogRecipeFn>;
